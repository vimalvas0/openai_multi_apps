...
let context: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [{ role: "system", content: "You are a helpful assistant." }];
...
function appendToContext(object: OpenAI.Chat.Completions.ChatCompletionMessageParam, desiredLength: number = CONTEXT_LENGTH) {
  ...
  // Filter out user context if the length exceeds the desired length
  if (context.length > desiredLength) {
    const userContext = context.filter((msg) => msg.role === "user");
    const nonUserContext = context.filter((msg) => msg.role !== "user");

    if (userContext.length > 0) {
      context.splice(0, userContext.length);
    } else {
      context.splice(0, context.length - desiredLength);
    }

    context = [...nonUserContext, ...context];
  }
